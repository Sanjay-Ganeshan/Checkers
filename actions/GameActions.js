...
exports.submitMovePlayer = {
  name: 'submitMove',
  description: 'I try submitting moves for authenticated players. If it is not their turn, I tell them so',
  blockedConnectionTypes: [],
  outputExample: {},
  matchExtensionMimeType: false,
  version: 1.0,
  ...
  run: function (api, data, next) {
    ...
    if (playerGame != null) {
      if (playerGame.isTurnOf(player)) {
          var worked = playerGame.move(data.params.x1,data.params.y1,data.params.x2,data.params.y2);
          if (worked) {
